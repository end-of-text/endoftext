...
<div class="flex h-full w-full items-center justify-center">
	<div class="flex w-60 flex-col items-center justify-center">
		...
		<form class="w-full" method="post" action="?/loginWithOauth" use:enhance>
			<button
		...
		{#if formType === 'login'}
			<form class="flex w-full flex-col gap-2" method="post" action="?/login" use:enhance>
		...
		{:else}
			<form class="flex w-full flex-col gap-2" method="post" action="?/signup" use:enhance>
				<input name="email" placeholder="email" value={form?.email ?? ''} />
				<input class="mb-2" type="password" name="password" placeholder="password" />
