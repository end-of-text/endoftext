	...
	<div class="h-1/2 w-40">
		...
		{#if formType === 'login'}
			<form class="flex flex-col gap-2" method="post" action="?/login" use:enhance>
				...
				<Button fancy classNames="ml-auto">Sign in</Button>
			</form>
		{:else}
			<form class="flex flex-col gap-2 self-end" method="post" action="?/signup" use:enhance>
				<input name="email" placeholder="email" value={form?.email ?? ''} />
				<input type="password" name="password" placeholder="password" />
				<Button fancy classNames="ml-auto">Sign up</Button>
			</form>
		{/if}
		{#if form?.success}
