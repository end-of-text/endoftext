...
<div class="flex w-full items-center border-b px-6">
	...
	<div class="ml-auto flex items-center gap-3">
		<div class="text-primary pointer-events-none rounded-md bg-rose-50 px-4 py-1 text-sm font-bold">
			Beta
		</div>
		<HelpButton />
		<button onclick={() => (showSettings = true)}>
			<Settings class="hover:text-primary h-5 w-5 cursor-pointer transition-all" />
		</button>
	</div>
</div>
