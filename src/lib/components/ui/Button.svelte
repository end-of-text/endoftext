...
{#if fancy}
	<div class={twMerge('group relative w-fit', classNames)}>
		...
		></div>
		<button
			class="animate relative rounded-lg bg-white px-4 py-2 hover:bg-gray-50 disabled:text-gray-500"
			{onclick}
			{disabled}
		>
			{@render children()}
...
{:else}
	<button
		class={twMerge(
			'flex gap-2 rounded border px-3 py-1.5 transition-all hover:bg-gray-100',
			classNames
		)}
		{disabled}
		{onclick}
